...
void modelInit()
{
  ...
  d->br_read_arr[2].id  = opened_b54;
    strncpy(d->br_read_arr[2].to_str, "B54 Opened", sizeof(d->br_read_arr[6].to_str));
  d->br_read_arr[3].id  = closed_b54;
    strncpy(d->br_read_arr[3].to_str, "B54 Closed", sizeof(d->br_read_arr[7].to_str));
  d->br_read_arr[4].id  = opened_b55;
    strncpy(d->br_read_arr[4].to_str, "B55 Opened", sizeof(d->br_read_arr[8].to_str));
  d->br_read_arr[5].id  = closed_b55;
    strncpy(d->br_read_arr[5].to_str, "B55 Closed", sizeof(d->br_read_arr[9].to_str));
  d->br_read_arr[6].id = -1;
  d->br_read_arr[7].id = -1;
  d->br_read_arr[8].id = -1;
  d->br_read_arr[9].id = -1;
  d->br_read_arr[10].id = -1;
  d->br_read_arr[11].id = -1;
  d->br_read_arr[12].id = -1;
  d->br_read_arr[13].id = -1;
  ...
  d->br_write_arr[2].id  = trip_b54;
    strncpy(d->br_write_arr[2].to_str, "Trip B54", sizeof(d->br_write_arr[6].to_str));
  d->br_write_arr[3].id  = close_b54;
    strncpy(d->br_write_arr[3].to_str, "Close B54", sizeof(d->br_write_arr[7].to_str));
  d->br_write_arr[4].id  = trip_b55;
    strncpy(d->br_write_arr[4].to_str, "Trip B55", sizeof(d->br_write_arr[8].to_str));
  d->br_write_arr[5].id  = close_b55;
    strncpy(d->br_write_arr[5].to_str, "Close B55", sizeof(d->br_write_arr[9].to_str));
  ...
  d->br_write_arr[10].id = -1;
  d->br_write_arr[10].type = -1;
  d->br_write_arr[11].id = -1;
  d->br_write_arr[11].type = -1;
  d->br_write_arr[12].id = -1;
  d->br_write_arr[12].type = -1;
  d->br_write_arr[13].id = -1;
  d->br_write_arr[13].type = -1;
...
static int slotNullEvent(PARAM *p, DATA *dptr)
{
  ...
  if (d->br_read_arr[6].value == 0) {
    pvSetImage(p, latency_image, "black.jpg");
  } else {
    pvSetImage(p, latency_image, "white.jpg");
  }
  /* Update the dials */
  for (int i = 0; i < NUM_POINT; i++) {
    if (d->point_arr[i].id == -1)
