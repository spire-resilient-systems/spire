...
void Process_Message(signed_message *mess) 
{
    ...
    for (int q = 0; q < NUM_BREAKER; ++q) {
        printf("Read Arr[%d] = %d\n", q, d->br_read_arr[q].value);
        printf("Write Arr[%d] = %d\n", q, d->br_write_arr[q].value);
    }
    for (int k = 0; k < NUM_POINT; ++k) {
        printf("Point Arr[%d] = %d\n", k, d->point_arr[k].value);
    }
    printf("\n\n");
...
void Button_Event(int dummy1, void *dummy2)
{
    ...
    if (Script_Breaker_Val == BREAKER_ON) {
        ...
        t = Next_Button;
        Script_Breaker_Val = BREAKER_ON;
        Script_Breaker_Index = (Script_Breaker_Index + 1) % 6; // MAGIC NUMBER
    }
