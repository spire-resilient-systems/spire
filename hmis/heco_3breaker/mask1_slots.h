...
void modelInit()
{
  ...
  d->point_arr[0].id = -1;
  d->point_arr[1].id = -1;
  d->point_arr[2].id = dial1;
  d->point_arr[3].id = dial2;
  ...
  d->br_read_arr[0].id  = opened_b101;
    strncpy(d->br_read_arr[0].to_str, "B10 Opened", sizeof(d->br_read_arr[0].to_str));
  d->br_read_arr[1].id  = closed_b101;
    strncpy(d->br_read_arr[1].to_str, "B10 Closed", sizeof(d->br_read_arr[1].to_str));
  ...
  d->br_read_arr[2].id = -1;
  d->br_read_arr[3].id = -1;
  d->br_read_arr[4].id = -1;
  d->br_read_arr[5].id = -1;
  d->br_read_arr[6].id = -1;
  d->br_read_arr[7].id = -1;
  d->br_read_arr[8].id = -1;
  d->br_read_arr[9].id = -1;
  d->br_read_arr[10].id  = opened_b54;
    strncpy(d->br_read_arr[10].to_str, "B54 Opened", sizeof(d->br_read_arr[6].to_str));
  d->br_read_arr[11].id  = closed_b54;
    strncpy(d->br_read_arr[11].to_str, "B54 Closed", sizeof(d->br_read_arr[7].to_str));
  d->br_read_arr[12].id  = opened_b55;
    strncpy(d->br_read_arr[12].to_str, "B55 Opened", sizeof(d->br_read_arr[8].to_str));
  d->br_read_arr[13].id  = closed_b55;
    strncpy(d->br_read_arr[13].to_str, "B55 Closed", sizeof(d->br_read_arr[9].to_str));
  ...
  d->br_write_arr[0].id  = trip_b101;
    strncpy(d->br_write_arr[0].to_str, "Trip B10", sizeof(d->br_write_arr[0].to_str));
  d->br_write_arr[1].id  = close_b101;
    strncpy(d->br_write_arr[1].to_str, "Close B10", sizeof(d->br_write_arr[1].to_str));
  ...
  d->br_write_arr[2].id = -1;
  d->br_write_arr[2].type = -1;
  d->br_write_arr[3].id = -1;
  d->br_write_arr[3].type = -1;
  d->br_write_arr[4].id = -1;
  d->br_write_arr[4].type = -1;
  d->br_write_arr[5].id = -1;
  d->br_write_arr[5].type = -1;
  d->br_write_arr[6].id = -1;
  d->br_write_arr[6].type = -1;
  d->br_write_arr[7].id = -1;
  d->br_write_arr[7].type = -1;
  d->br_write_arr[8].id = -1;
  d->br_write_arr[8].type = -1;
  d->br_write_arr[9].id = -1;
  d->br_write_arr[9].type = -1;
  d->br_write_arr[10].id  = trip_b54;
    strncpy(d->br_write_arr[10].to_str, "Trip B54", sizeof(d->br_write_arr[6].to_str));
  d->br_write_arr[11].id  = close_b54;
    strncpy(d->br_write_arr[11].to_str, "Close B54", sizeof(d->br_write_arr[7].to_str));
  d->br_write_arr[12].id  = trip_b55;
    strncpy(d->br_write_arr[12].to_str, "Trip B55", sizeof(d->br_write_arr[8].to_str));
  d->br_write_arr[13].id  = close_b55;
    strncpy(d->br_write_arr[13].to_str, "Close B55", sizeof(d->br_write_arr[9].to_str));
...
static int slotInit(PARAM *p, DATA *dptr)
{
  ...
  // Set the middle breaker from the 7 breaker scenario to open
  printf("Sending!\n\n");
  gettimeofday(&dptr->button_press_time, NULL);
  ps.incarnation = My_Incarnation;
  ps.seq_num = Seq_Num;
  mess = PKT_Construct_HMI_Command_Msg(ps, MAX_EMU_RTU + My_ID, PNNL, BREAKER_ON, 4); 
  ...
  ps.incarnation = My_Incarnation;
  ps.seq_num = Seq_Num;
  mess = PKT_Construct_HMI_Command_Msg(ps, MAX_EMU_RTU + My_ID, PNNL, BREAKER_OFF, 4); 
  mess->global_configuration_number=My_Global_Configuration_Number;
  nbytes = sizeof(signed_message) + mess->len;
  Seq_Num++;
  IPC_Send(ipc_sock, (void *)mess, nbytes, itrc_in.ipc_remote);
