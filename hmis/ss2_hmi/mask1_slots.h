...
void modelInit()
{
  ...
  for (int i = 0; i < SUBSTATION_NUM_POINT; i++) {
    ...
    d->point_arr[9].id = box_10;
  
    d->load_dial_arr[0].type=DIAL;
    d->load_dial_arr[0].id=dial_l1;
    d->load_dial_arr[1].id=dial_l2;
    d->load_dial_arr[1].type=DIAL;
  ...
  for (int i = 0; i < SUBSTATION_NUM_POINT; i++) {
    ...
    d->load_dial_arr[i].value=-1;
    //breaker state unknown , so load set to 0
