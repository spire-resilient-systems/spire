...
void Init_SM_Replicas()
{
    ...
    {   
        // MK: The following algorithm works for 6_6_5_4
        ...
        else
        {
            ...
            if(dc_rep == (NUM_SM - NUM_CC_REPLICA))
            {
                site = NUM_SITES - 1;
            }
        }
        ...
        if(cc_rep < NUM_CC_REPLICA && dc_rep < (NUM_SM - NUM_CC_REPLICA))
        {
            site = (site + 1) % NUM_SITES;
        }
        else if(cc_rep < NUM_CC_REPLICA)
        {
            site = (site + 1) % NUM_CC;
        }
        else // dc_rep <= (NUM_SM - NUM_CC_REPLICA)
        {
            site = NUM_CC + ((site + 1) % (NUM_SITES - NUM_CC)) ;
        } 

        // MK: Old code (Keeping it here just for comparison)
        // All_Sites[id-1] = site;
