...
//extern int32u My_curr_global_config_num;
//FILE              *fw;
//char              fw_buf[36000];

/* Local Functions */
void Print_Statistics();
//void Init_ctrl_spines();
//void Handle_Config_Msg();
...
void Process_Config_Msg(signed_message * conf_mess,int mess_size){
    ...
    //printf("Got reconf msg at %lu\t%lu\n", reconf_st.tv_sec, reconf_st.tv_usec);
    print_reconf=1;
    ...
    if (ret!=mess_size) printf("Conf msg to ITRC Remote: IPC_Send error!\n");
    	//printf("Sent conf msg of size %d to itrc.remote\n",ret);
    //printf("Reconf done \n");
    sleep(My_ID * 0.1);
}
...
void Handle_Config_Msg(){
    ...
    ret=spines_recvfrom(ctrl_spines, buff, MAX_LEN, 0, (struct sockaddr *) &from_addr, &from_len);
    printf("Benchmark: Received spines message of size=%d from %s\n",ret,inet_ntoa(from_addr.sin_addr));
    if (ret<=0){
        perror( "Benchmark: Disconnected by spines....\n");
    }
    if (ret < sizeof(signed_message)){
        printf("Benchmark: Config Message size smaller than signed message\n");
        return;
    }
    mess = (signed_message*)buff;

    if(mess->global_configuration_number<=My_curr_global_config_num){
        printf("Receive mess with lower global conf number:%u and my config is %u\n",mess->global_configuration_number,My_curr_global_config_num);
        return;
    }
    if (ret < (sizeof(signed_message)+mess->len)){
        printf("Benchmark: Config Message size smaller than expected\n");
        return;
    }
    if (from_addr.sin_addr.s_addr != inet_addr(SPINES_HMI_ADDR)) {
        printf("Benchmark: Message is not from Config Manager Address\n");
        return;
    }
    if(mess->type!=PRIME_OOB_CONFIG_MSG){
        printf("Benchmark: Message type is not config message\n");
        return;
    }
    ...
    printf("Verified Config Message\n");
    My_curr_global_config_num = mess->global_configuration_number;
...
/*
void Init_ctrl_spines(){
    char *ctrl_spines_addr = itrc_thread.spines_ext_addr;
    ctrl_spines=Spines_Mcast_SendOnly_Sock(ctrl_spines_addr, SPINES_CTRL_PORT, SPINES_PRIORITY);
    if (ctrl_spines < 0 ) {
        printf( "Benchmark: Error setting up control spines network, exiting\n");
    }

    printf("Benchmark: Connected to control spines\n");
    name.sin_family = AF_INET;
    name.sin_addr.s_addr = htonl(INADDR_ANY);
    name.sin_port = htons(CTRL_SPINES_MCAST_PORT);
    if(spines_bind(ctrl_spines, (struct sockaddr *)&name, sizeof(name) ) < 0) {
      perror("Benchmark: bind error \n");
    }
...
void Gen_Msg() 
{
    ...
    free(mess);
    //printf("Sent msg of size %d with seq=%d of gcn=%lu\n",ret,seq.seq_num-1,My_Global_Configuration_Number);
}
...
void Process_Msg()
{
    ...
    //printf("process msg recieved %d bytes\n",ret);
...
int main(int argc,char *argv[])
{
    ...
    //Initialize connection to control spines
    //Init_ctrl_spines();
    ...
    //Setup the FD_SET for ctrl spines
    //FD_SET(ctrl_spines,&mask);
