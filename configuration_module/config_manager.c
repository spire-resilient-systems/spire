...
void construct_key_message(){
    ...
    key_messages[total_key_frags] = (signed_message *) PKT_Construct_Signed_Message(sizeof(key_msg_header)+(curr_idx*sizeof(char)));
    //Fill main header
...
void read_pvt_key(char * filename,int type, int id){
    ...
    if (type==PRIME_RSA_PVT){
    	sprintf(enc_key_filename,"../prime/bin/tpm_keys/tpm_public%d.pem",id);
    }
    if(type==SM_TC_PVT || type == PRIME_TC_PVT){
	sprintf(enc_key_filename,"../prime/bin/tpm_keys/tpm_public%d.pem",id+1);
    }
...
int main(int argc, char **argv)
{
    ...
    Usage(argc,argv);
    OPENSSL_RSA_Init();
    OPENSSL_RSA_Read_Keys(0,RSA_CONFIG_MNGR,SM_PRIME_KEYS);
    Init_Network();
...
void Init_Network()
{
    ctrl_spines=Spines_Mcast_SendOnly_Sock(SPINES_HMI_ADDR, SPINES_CTRL_PORT, SPINES_PRIORITY);
...
void new_broadcast_configuration_message(int code, void *dummy)
{
    ...
    mess=PKT_Construct_OOB_Config_Msg();
    now=E_get_time();
    ...
    conf_msg=(config_message *)(mess+1);
    memset(filename,0,sizeof(filename));
   ...
   len=0;
    ...
    conf_msg->spines_ext_port=SPINES_INT_PORT;
    fclose(fp1);
    if (line2){
